...
import com.jl.hl.sinter.plc.moka7.IntByRef;
import com.jl.hl.sinter.plc.moka7.S7;
import com.jl.hl.sinter.plc.moka7.S7BlockInfo;
import com.jl.hl.sinter.plc.moka7.S7Client;
import com.jl.hl.sinter.plc.moka7.S7CpInfo;
import com.jl.hl.sinter.plc.moka7.S7CpuInfo;
import com.jl.hl.sinter.plc.moka7.S7OrderCode;
import com.jl.hl.sinter.plc.moka7.S7Protection;
import com.jl.hl.sinter.plc.moka7.S7Szl;
...
public class ClientDemo {
	...
	private static byte[] Buffer = new byte[65536]; // 64K buffer (maximum for
													// S7400 systems)
	private static final S7Client Client = new S7Client();
	private static int ok = 0;
	private static int ko = 0;
	private static String IpAddress = "";
	private static int Rack = 0; // must change for S7400
	private static int Slot = 2; // must change for S7400
	private static int DBSample = 8; // Sample DB that must be present in the
										// CPU
	...
	static void BlockInfo(int BlockType, int BlockNumber) {
		...
		if (Result == 0) {
			System.out.println("Block Flags     : "
					+ Integer.toBinaryString(Block.BlkFlags()));
			...
			System.out.println("Checksum        : 0x"
					+ Integer.toHexString(Block.Checksum()));
			SimpleDateFormat ft = new SimpleDateFormat("dd/MM/yyyy");
			System.out.println("Code Date       : "
					+ ft.format(Block.CodeDate()));
			System.out.println("Interface Date  : "
					+ ft.format(Block.IntfDate()));
		}
		TestEnd(Result);
	}
	...
	public static boolean DBGet() {
		...
		if (Result == 0) {
			...
			System.out.println("DB " + DBSample + " - Size read " + DataToMove
					+ " bytes");
	...
	public static void DBRead() {
		TestBegin("ReadArea()");
		int Result = Client.ReadArea(S7.S7AreaDB, DBSample, 0, DataToMove,
				Buffer);
		if (Result == 0) {
			System.out.println("DB " + DBSample
					+ " succesfully read using size reported by DBGet()");
		}
		TestEnd(Result);
	}
	...
	public static void DBWrite() {
		TestBegin("WriteArea()");
		int Result = Client.WriteArea(S7.S7AreaDB, DBSample, 0, DataToMove,
				Buffer);
		if (Result == 0) {
			System.out.println("DB " + DBSample
					+ " succesfully written using size reported by DBGet()");
		}
		TestEnd(Result);
	}
	...
	public static void GetSysInfo() {
		...
		if (Result == 0) {
			...
			System.out.println("Firmware version  : " + OrderCode.V1 + "."
					+ OrderCode.V2 + "." + OrderCode.V3);
		}
		TestEnd(Result);
		...
		if (Result == 0) {
			System.out.println("Module Type Name  : "
